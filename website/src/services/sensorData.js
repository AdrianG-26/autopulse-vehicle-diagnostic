...
class SensorDataService {
  ...
  async getLatestSensorData(vehicleId = DEFAULT_VEHICLE_ID) {
    ...
    try {
      ...
      if ((!data || data.length === 0 || (error && error.code === 'PGRST301')) && vehicleId === DEFAULT_VEHICLE_ID) {
      ...
      if (error && error.code !== 'PGRST116') { // PGRST116 = no rows, which is acceptable
        ...
        if (error.code === 'PGRST301' || error.message?.includes('permission') || error.message?.includes('policy')) {
          console.warn('RLS policy may be blocking access. Check Supabase RLS policies or use service role key for development.');
        }
      }
      ...
      console.log('📊 Raw sensor data from Supabase:', {
        ...
        hasHealthStatus: rawData.health_status !== null && rawData.health_status !== undefined,
      ...
      if (rawData.rpm === null || rawData.rpm === undefined || rawData.rpm === 0) {
  ...
  normalizeSensorData(raw) {
    ...
    return {
      ...
      mlHealthScore: parseFloat(raw.ml_health_score || raw.mlHealthScore || raw.health_score || raw.healthScore) || null,
      mlStatus: raw.ml_status || raw.mlStatus || null,
      mlAlerts: Array.isArray(raw.ml_alerts) ? raw.ml_alerts : 
                (Array.isArray(raw.mlAlerts) ? raw.mlAlerts : 
                (raw.ml_alerts ? [raw.ml_alerts] : 
                (raw.mlAlerts ? [raw.mlAlerts] : []))),
      mlConfidence: parseFloat(raw.ml_confidence || raw.mlConfidence || raw.confidence) || null,
  ...
  subscribeToSensorData(vehicleId, callback) {
    ...
    this.getLatestSensorData(vehicleId).then(data => {
    ...
    console.log(`🔄 Setting up periodic polling backup (every 3 seconds) for vehicle ${vehicleId}`);
    const pollingInterval = setInterval(async () => {
      console.log(`🔄 Periodic polling - fetching data for vehicle ${vehicleId}`);
      try {
    ...
    try {
      ...
      const channel = supabase
        ...
        .on(
          ...
          (payload) => {
            ...
            if (eventType === 'INSERT' || eventType === 'UPDATE') {
              console.log(`🔄 Realtime triggered - fetching latest data for vehicle ${vehicleId}`);
              this.getLatestSensorData(vehicleId).then(data => {
        ...
        .subscribe((status) => {
          ...
          if (status === 'SUBSCRIBED') {
            console.log(`✅ Successfully subscribed to real-time updates for vehicle ${vehicleId}`);
          } else if (status === 'CHANNEL_ERROR') {
            console.error('❌ Realtime channel error. Falling back to periodic REST API calls.');
            // Fallback: If Realtime fails, use periodic REST API calls as backup
            const fallbackInterval = setInterval(async () => {
              console.log(`🔄 Fallback polling - fetching data for vehicle ${vehicleId}`);
              const data = await this.getLatestSensorData(vehicleId);
              if (data && callback) {
            ...
            console.warn(`⚠️ Realtime subscription ${status}. Setting up fallback polling.`);
            // Set up fallback polling if Realtime times out or closes
            const fallbackInterval = setInterval(async () => {
              console.log(`🔄 Fallback polling (${status}) - fetching data for vehicle ${vehicleId}`);
              const data = await this.getLatestSensorData(vehicleId);
              if (data && callback) {
                callback(data);
              }
      ...
      console.log('🔄 Falling back to periodic REST API calls every 3 seconds...');
      const fallbackInterval = setInterval(async () => {
        console.log(`🔄 Fallback polling (catch block) - fetching data for vehicle ${vehicleId}`);
        const data = await this.getLatestSensorData(vehicleId);
        if (data && callback) {
          console.log('✅ Fallback polling - data received and callback called');
          callback(data);
        } else {
          console.warn('⚠️ Fallback polling - no data received');
        }
