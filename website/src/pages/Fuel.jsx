  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        if (data && data.length > 0) { const latest = data[0]; setRawData({ fuel_level: latest.fuel_level, throttle_pos: latest.throttle_pos }); setPollingStatus("CONNECTED"); } else { setPollingStatus("WAITING"); }
        return;
        const API_BASE_OLD =
