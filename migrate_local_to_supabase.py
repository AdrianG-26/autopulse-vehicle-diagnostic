...
=========================================
Uploads all existing OBD data from Raspberry Pi to Supabase cloud
"""
    ...
    try:
        ...
        if existing.data:
            ...
            profile_data = {
                ...
                'total_sessions': vehicle.get('total_sessions', 0),
    ...
    for session_row in sessions:
        ...
        while offset < session_rows:
            try:
                ...
                for row_obj in rows:
                    row = dict(row_obj)
                    # Map to telemetry_data format
                    telemetry_record = {
                        'vehicle_id': str(supabase_id),  # Use Supabase vehicle_profiles ID
                        'timestamp': row['timestamp'],
                        'rpm': row.get('rpm'),
                        'speed': row.get('vehicle_speed'),
                        'coolant_temp': row.get('coolant_temp'),
                        'battery': row.get('control_module_voltage'),
                        'engine_load_pct': row.get('engine_load'),
                        'throttle_position': row.get('throttle_pos'),
                        'fuel_level_pct': row.get('fuel_level'),
                        'map_kpa': row.get('map'),
                        'intake_air_temp': row.get('intake_temp'),
                        'ignition_advance': row.get('timing_advance'),
                        'fuel_trim_short_pct': row.get('fuel_trim_short'),
                        'fuel_trim_long_pct': row.get('fuel_trim_long')
                    }
                ...
                if failed_batches > 10:
                    print("      ⚠️  Too many failed batches, skipping this session")
                    break
