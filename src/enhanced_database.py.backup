        ...
        try:
            with self._get_connection() as conn:
                ...
                for _, row in df.iterrows():
                    ...
                    print(f"🕐 {row['timestamp']} | 🚗 {car_info}")
                    print(f"   {status_emoji} Status: {row['status']:8} | RPM: {row['rpm']:6.0f} | Load: {row['engine_load']:5.1f}%")
                    print(f"   🌡️  Temp: {row['coolant_temp']:5.1f}°C | 🚀 Speed: {row['vehicle_speed']:5.1f} km/h | ⛽ Fuel: {row['fuel_level']:5.1f}%")
                    print(f"   📈 Stress: {row['engine_stress_score']:5.1f} | ⚠️  Fault: {row['fault_type']}")
                    print()
